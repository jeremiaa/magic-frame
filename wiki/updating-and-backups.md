...
> **Read this before going backwards.** The application brings the database
> schema in line with its own expectations every time the container starts, and
> it is allowed to drop things to do so. Moving *forward* is what that is
> designed for. Moving *back* across a release that changed the schema can
> therefore lose the data belonging to whatever was added in between. Take a
> database dump first, every time, and prefer reporting the problem to living on
> an old version.
